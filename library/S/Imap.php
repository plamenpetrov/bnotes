...
class S_Imap
{
	...
	public function __construct($config, $folder)
	{
        ...
        $mail = new S_Mail_Storage_Imap(array(
            'host'	   => $config['host'],
            'user'     => $config['user'],
     ...
     */
	public static function getInstance($config, $folder = 'INBOX')
	{
		if (null === self::$_instance) {
			self::$_instance = new self($config, $folder);
		}
	...
	public function getRecursiveFolderList($folders, $options = array())
	{
        ...
        foreach ($folders as $localName => $folder) {
        	$folderKey = S_Utilities::sanitizeId($folder);
        	...
        	$listItem = array_merge($options[$folderKey], array(
                    'text'        => htmlspecialchars(S_Decode::UTF7($localName)),
                	...
                	'classConfig' => array_merge($options[$folderKey]['classConfig'], array(
                		...
                		'title'	  => htmlspecialchars(S_Decode::UTF7($localName))
                	))
        	));
	...
	public function removeMessage($id)
	{
	    try {
	        ...
	        if ($current != 'INBOX'.MAIL_IMAP_DELIMITER.'Trash') {
    	        $this->setFolder('INBOX'.MAIL_IMAP_DELIMITER.'Trash');
    	        $this->setFolder($current);
    	        $this->getMail()->copyMessage($id, 'INBOX'.MAIL_IMAP_DELIMITER.'Trash');
	...
	public function moveMessageToJunk($id)
	{
	    try {
	        $current = $this->getMail()->getCurrentFolder();
	        if ($current != 'INBOX'.MAIL_IMAP_DELIMITER.'Junk') {
	            $this->setFolder('INBOX'.MAIL_IMAP_DELIMITER.'Junk');
	            $this->setFolder($current);
	            $this->getMail()->moveMessage($id, 'INBOX'.MAIL_IMAP_DELIMITER.'Junk');
	        }
	        return true;
	    } catch (Zend_Mail_Storage_Exception $e) {
	        return false;
	...
	public function convertMessage($message)
	{
        ...
        $str = S_Utilities::utf8Encode($str, S_Utilities::getCharsetFromContentType(($message->headerExists('content-type') ? $message->contentType : '')));
        return $str;
	}
	...
	public function cleanSubject($str){
	    if(strstr($str, '=?UTF-8?B?')){
	    	$subject  = str_replace('=?UTF-8?B?' , '' , $str);
	    	$subject  = str_replace('?=' , '' , $subject);
	    	$subject  = base64_decode($subject);
	    } elseif(strstr($str, '=?UTF-8?Q?')) {
	    	$subject  = str_replace('=?UTF-8?Q?' , '' , $str);
	    	$subject  = str_replace('?=' , '' , $subject);
	    	$subject  = quoted_printable_decode($str);
	    } elseif(strstr($str, '=?ISO-8859-5?B?')){
	        $subject  = str_replace('=?ISO-8859-5?B?' , '' , $str);
	        $subject  = str_replace('?=' , '' , $subject);
	        $subject  = base64_decode($str);
	    } else {
	    	$subject= imap_utf8($str);
	    }
	    return $subject;
	}
	...
	{
    	$key = S_Utilities::sanitizeId($this->_getCachePrefix() . '_' . $id);
